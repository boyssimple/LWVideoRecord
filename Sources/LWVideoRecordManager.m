...
#import "LWVideoRecordEncoder.h"
#import <AVFoundation/AVFoundation.h>
#import <Photos/Photos.h>

@interface LWVideoRecordManager ()<AVCaptureVideoDataOutputSampleBufferDelegate,AVCaptureAudioDataOutputSampleBufferDelegate, CAAnimationDelegate> {
...
@property (strong, nonatomic) LWVideoRecordEncoder           *recordEncoder;//录制编码
...
@implementation LWVideoRecordManager
...
- (void) stopCaptureHandler:(void (^)(UIImage *movieImage))handler {
    [self stopCapture:TRUE handler:handler];
}

//停止录制
- (void) stopCapture:(BOOL)save handler:(void (^)(UIImage *movieImage))handler {
    @synchronized(self) {
        if (self.isCapturing) {
...
- (void) captureOutput:(AVCaptureOutput *)captureOutput didOutputSampleBuffer:(CMSampleBufferRef)sampleBuffer fromConnection:(AVCaptureConnection *)connection {
    ...
    @synchronized(self) {
        ...
        if ((self.recordEncoder == nil) && !isVideo) {
            ...
            self.recordEncoder = [LWVideoRecordEncoder encoderForPath:self.videoPath Height:_cy width:_cx channels:_channels samples:_samplerate];
        }
        //判断是否中断录制过
        if (self.discont) {
            if (isVideo) {
